#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <pthread.h>
#include <assert.h>
#include <sys/time.h>

#define NUM_BUCKETS 5     // Buckets in hash table
#define NUM_KEYS 100000   // Number of keys inserted per thread
int num_threads = 1;      // Number of threads (configurable)
int keys[NUM_KEYS];

// Struct to accomodate multiple locks
struct lock_array
{
  pthread_mutex_t lock;
};

//array of locks. 5 since there are 5 buckets
struct lock_array lck_arr[5];


//Create mutexes here. 5 mutexes, 1 for each bucket to achieve parallelism

pthread_mutex_t mut0;
pthread_mutex_t mut1;
pthread_mutex_t mut2;
pthread_mutex_t mut3;
pthread_mutex_t mut4;



typedef struct _bucket_entry {
  int key;
  int val;
  struct _bucket_entry *next;
} bucket_entry;

bucket_entry *table[NUM_BUCKETS];

void panic(char *msg) {
  printf("%s\n", msg);
  exit(1);
}

double now() {
  struct timeval tv;
  gettimeofday(&tv, 0);
  return tv.tv_sec + tv.tv_usec / 1000000.0;
}

// Inserts a key-value pair into the table
void insert(int key, int val) {

  //int j = key % NUM_BUCKETS;
  //pthread_mutex_lock(&lck_arr[j].lock);

  //Lock before inserting to prevent key loss.
  /* To achieve parallelism, 
  each table will have a unique lock for the respective threads.
  Respective threads can insert in the bucket in parallel*/

  int i = key % NUM_BUCKETS;
  pthread_mutex_lock(&lck_arr[i].lock);
  bucket_entry *e = (bucket_entry *) malloc(sizeof(bucket_entry));
  if (!e) panic("No memory to allocate bucket!");
  e->next = table[i];
  e->key = key;
  e->val = val;
  table[i] = e;
  //Release lock once completed
  pthread_mutex_unlock(&lck_arr[i].lock);
}

// Retrieves an entry from the hash table by key
// Returns NULL if the key isn't found in the table
bucket_entry * retrieve(int key) {
  //Lock before retrieval to prevent key loss
  /* To achieve parallelism, 
  each table will have a unique lock for the respective threads.
  Respective threads can retrieve in the bucket in parallel*/
  
  int j = key % NUM_BUCKETS;
  pthread_mutex_lock(&lck_arr[j].lock);

  bucket_entry *b;
  for (b = table[key % NUM_BUCKETS]; b != NULL; b = b->next) {
    if (b->key == key) { 
      //Release lock once key is found and return the retrieved bucket entry
      pthread_mutex_unlock(&lck_arr[j].lock); 
      return b;
    }
  }
  //Release lock once completed
  pthread_mutex_unlock(&lck_arr[j].lock);
  return NULL;
}

void * put_phase(void *arg) {
  long tid = (long) arg;
  int key = 0;

  // If there are k threads, thread i inserts
  //      (i, i), (i+k, i), (i+k*2)
  for (key = tid ; key < NUM_KEYS; key += num_threads) {
    insert(keys[key], tid);
  }

  pthread_exit(NULL);
}

void * get_phase(void *arg) {
  long tid = (long) arg;
  int key = 0;
  long lost = 0;

  for (key = tid ; key < NUM_KEYS; key += num_threads) {
    if (retrieve(keys[key]) == NULL) lost++;
  }
  printf("[thread %ld] %ld keys lost!\n", tid, lost);

  pthread_exit((void *)lost);
}

int main(int argc, char **argv) {
  long i;
  pthread_t *threads;
  double start, end;

  if (argc != 2) {
    panic("usage: ./parallel_hashtable <num_threads>");
  }
  if ((num_threads = atoi(argv[1])) <= 0) {
    panic("must enter a valid number of threads to run");
  }

  if (pthread_mutex_init(&mut0, NULL) != 0) {
      printf("\n Mutex0 init has failed\n");
      return 1;
  }
  
  if (pthread_mutex_init(&mut1, NULL) != 0) {
      printf("\n Mutex1 init has failed\n");
      return 1;
  }

  if (pthread_mutex_init(&mut2, NULL) != 0) {
      printf("\n Mutex2 init has failed\n");
      return 1;
  }

  if (pthread_mutex_init(&mut3, NULL) != 0) {
      printf("\n Mutex3 init has failed\n");
      return 1;
  }

  if (pthread_mutex_init(&mut4, NULL) != 0) {
      printf("\n Mutex4 init has failed\n");
      return 1;
  }

  // Initialize array of locks.
  lck_arr[0].lock = mut0;
  lck_arr[1].lock = mut1;
  lck_arr[2].lock = mut2;
  lck_arr[3].lock = mut3;
  lck_arr[4].lock = mut4;

  srandom(time(NULL));
  for (i = 0; i < NUM_KEYS; i++)
    keys[i] = random();

  threads = (pthread_t *) malloc(sizeof(pthread_t)*num_threads);
  if (!threads) {
    panic("out of memory allocating thread handles");
  }

  // Insert keys in parallel
  start = now();
  for (i = 0; i < num_threads; i++) {
    pthread_create(&threads[i], NULL, put_phase, (void *)i);
  }

  // Barrier
  for (i = 0; i < num_threads; i++) {
    pthread_join(threads[i], NULL);
  }
  end = now();

  printf("[main] Inserted %d keys in %f seconds\n", NUM_KEYS, end - start);

  // Reset the thread array
  memset(threads, 0, sizeof(pthread_t)*num_threads);

  // Retrieve keys in parallel
  start = now();
  for (i = 0; i < num_threads; i++) {
    pthread_create(&threads[i], NULL, get_phase, (void *)i);
  }

  // Collect count of lost keys
  long total_lost = 0;
  long *lost_keys = (long *) malloc(sizeof(long) * num_threads);
  for (i = 0; i < num_threads; i++) {
    pthread_join(threads[i], (void **)&lost_keys[i]);
    total_lost += lost_keys[i];
  }
  end = now();

  printf("[main] Retrieved %ld/%d keys in %f seconds\n", NUM_KEYS - total_lost, NUM_KEYS, end - start);

  return 0;
}
